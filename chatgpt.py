...
def check_translation(message, english_word, russian_translation, randomrow, part):
    ...
    if user_translation == russian_translation.lower():
        ...
        bot.send_message(message.chat.id, "Записываю это слово в ваш личный словарь...")
        WriteWord(message, randomrow)
        ShowDict(message)
         # Задаем новое слово
        ChooseWord(message, part)

   
def WriteWord(message, randomrow):
    user_name = message.from_user.first_name
    file_toexcel = "translations"+user_name+".xlsx"

    if os.path.isfile(file_toexcel):
        existing = pd.read_excel("translations"+user_name+".xlsx")
        df_updated = pd.concat([existing, randomrow], ignore_index=True)
        df_updated.to_excel(file_toexcel, index=False)
    else:
        randomrow = pd.DataFrame(randomrow)
        randomrow.to_excel(file_toexcel, index = False)

def ShowDict(message):
    user_name = message.from_user.first_name
    dataF = pd.read_excel("translations"+user_name+".xlsx")
    dataF = dataF.iloc[:, :2]
    dataStr = dataF.to_string(index = False)
    bot.send_message(message.chat.id, dataStr)
